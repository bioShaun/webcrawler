...
URL = 'http://dkxy.nwafu.edu.cn/szdw/jsyjy/index.htm'
...
class Spider(base_spider_v3.Spider):
    ...
    def get_name_title(self):
        ...
        if name_tb_inf:
            name_tb = name_tb_inf.findAll('a', {'href': re.compile('sm.nwsuaf.edu.cn')})
            for n, each in enumerate(name_tb):
                ...
                if not each_inf:
                    continue
                try:
                    teacher_title = each_inf[2].get_text()
